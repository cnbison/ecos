...
def test_pomdp_transition_matrix_row_sums_to_1():
    """transition matrix 每行和 = 1 (valid stochastic matrix)."""
    policy = POMDPPolicy(n_arms=10, n_states=4, seed=42)
    row_sums = policy.transition.sum(axis=1)
    assert np.allclose(row_sums, np.ones(policy.n_states))
...
def test_pomdp_bayes_update_normalizes_belief():
    """bayes_update 后 belief_state 和 = 1."""
    policy = POMDPPolicy(n_arms=10, n_states=4, seed=42)
    policy.bayes_update(observation=2)  # Bored
    assert abs(policy.belief_state.sum() - 1.0) < 1e-9
    assert np.all(policy.belief_state >= 0)


def test_pomdp_bayes_update_increases_diagonal_state_probability():
    """bayes_update(obs=s) 应该增加 belief_state[s] (对角线观察)."""
    policy = POMDPPolicy(n_arms=10, n_states=4, seed=42)
    # 初始 uniform
    initial_belief = policy.belief_state.copy()
    # 多次 update obs=0 (Engaged) → belief[Engaged] 应该增加
    for _ in range(5):
        policy.bayes_update(observation=0)
...
def test_pomdp_bayes_update_invalid_observation_ignored():
    """bayes_update(越界 obs) → 跳过, belief_state 不变."""
    policy = POMDPPolicy(n_arms=10, n_states=4, seed=42)
    initial_belief = policy.belief_state.copy()
    initial_obs_count = policy.total_observations
    policy.bayes_update(observation=99)  # 越界
...
def test_pomdp_dump_load_roundtrip():
    ...
    policy.bayes_update(observation=1)
...
def test_pomdp_belief_state_over_steps():
    """belief_state 50 步内稳定 (不和发散, 不 NaN)."""
    policy = POMDPPolicy(n_arms=10, n_states=4, seed=42)
    for obs in [0, 1, 2, 3] * 13:  # 52 步
        policy.bayes_update(observation=obs)
    # belief_state 和 = 1
    assert abs(policy.belief_state.sum() - 1.0) < 1e-9
    # belief_state 不 NaN
...
def test_pomdp_seed_determinism():
    """固定 seed → bayes_update + select_arm 序列可重现."""
    policy1 = POMDPPolicy(n_arms=10, n_states=4, seed=42)
    policy2 = POMDPPolicy(n_arms=10, n_states=4, seed=42)
    arms1 = []
    arms2 = []
    for obs in [0, 1, 2, 3] * 5:
        policy1.bayes_update(observation=obs)
        policy2.bayes_update(observation=obs)
