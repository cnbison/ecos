"""ECOS Plugin SDK 5 Use Case 示例 (v0.91.0-e).

对应设计: docs/plugin_sdk.md §八.
...
def use_case_hint_fatigue_detection(bus: EventBus, lca_engine: Any) -> str:
    """Use Case 3: 提示疲劳检测 (hint_requested > 5).

    订阅 hint_requested topic, 计数 → 当 hint > 5 时提示教师学生可能过度依赖 hint.
    """
    subscription_id = bus.subscribe("hint_requested", _hint_fatigue_handler)

    def _hint_fatigue_handler(event: LearningEvent) -> None:
        """Hint fatigue handler: 计数 hint_requested → 触发 alert."""
        try:
            student_id = event.student_id
            entry = HumanFeedbackEntry.from_event(event)
            state = _get_or_create_belief_state(student_id)
            ...
            twin: Optional[CognitiveTwinAgent] = lca_engine._cognitive_twin.get(student_id)
            if twin is not None:
                hint_count = twin.human_feedback.count_by_type("hint_requested")
                if hint_count > 5:
                    _log.warning(
                        "提示疲劳告警 (sid=%s): hint_requested=%d > 5, "
                        "学生可能过度依赖 hint, 建议教师介入",
                        student_id, hint_count,
                    )
        except Exception:
            _log.warning("Hint fatigue handler 异常, skip", exc_info=True)
...
def register_all_use_cases(bus: EventBus, lca_engine: Any, llm_client: Any = None) -> Dict[str, str]:
