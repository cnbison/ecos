...
- **状态**：✅ 已落地（v0.98.8，Bisen 拍板执行）：12 道写代码题 rubric 补齐（回归测试宽判据又抓出 4 道漏网）+ PC-C04 历史缺陷顺手修 + prompt 双分支防幻觉 + 5 个测试；lbc 的 PB-Q16 记录已校正（轨迹 + evidence）。⚠️ 遗留注：增量信念状态已消费过 0 分观测，贝叶斯更新不可逆——dogfood 账号 lbc 建议重置后再继续（8 题轨迹成本低，换来干净状态；由 Bisen 决定）
