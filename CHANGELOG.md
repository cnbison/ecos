...
| 0.32.0 | 2026-07-04 | （待提交）| **M2 W3 LLM Critic 完整集成 + 持久化骨架**：LLM Critic 部分：实现 research/10-engineering/01-cta-belief-engine.md §9 LLM Critic 边界——`ecos/cta/llm_critic/{__init__.py, schemas.py, perception.py, explanation.py, misconception_detector.py}`（感知层 PerceptionCritic + 解释层 ExplanationCritic + Misconception 检测 MisconceptionDetector，3 类 prompt 严格遵守 §9 边界；温度 0.2；复用 ECOSLLMClient.chat_json）+ `ecos/cta/content/{__init__.py, misconceptions.py, threshold_concepts.py}`（30 条初中数学 Misconception 库 M1-M30；8 条 Threshold Concept 库 TC_function~TC_limit）+ `ecos/bloom/subject_libraries/math.py`（32 条 Bloom 目标 8 知识点×4 层 L1-L4）+ BeliefEngine.update() 集成：Step 5-6 LLM Critic 感知 + C 维度折扣；BeliefState.C 修正为 ConfidenceDimensionState。持久化部分：实现 research/10-engineering/05-persistence-session.md §2-4 MVP 范围——`ecos/persistence/db.py`（6 张 SQLite 表 + Database 主接口）+ `ecos/session/ecos_session.py`（ECOSSession 主类 + 自动保存 + epoch 快照）+ `ecos/session/chunk_isolation.py`（chunk 滚动计数器，每 threshold_epochs 强制快照） |
