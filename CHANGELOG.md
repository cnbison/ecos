...
| 0.32.0 | 2026-07-03 | （待提交）| **M2 W3 LLM Critic 完整集成**：实现 research/10-engineering/01-cta-belief-engine.md §9 LLM Critic 边界——`ecos/cta/llm_critic/{__init__.py, schemas.py, perception.py, explanation.py, misconception_detector.py}`（感知层 PerceptionCritic + 解释层 ExplanationCritic + Misconception 检测 MisconceptionDetector，3 类 prompt 严格遵守 §6.2 边界；温度 0.2；复用 ECOSLLMClient.chat_json）+ `ecos/cta/content/{__init__.py, misconceptions.py, threshold_concepts.py}`（30 条初中数学 Misconception 库 M1-M30，覆盖代数/几何/函数/统计/不等式 6 大类；8 条 Threshold Concept 库 TC_function~TC_limit）+ `ecos/bloom/subject_libraries/math.py`（32 条 Bloom 目标 8 知识点×4 层 L1-L4）+ BeliefEngine.update() 集成：Step 5 LLM 感知层更新 BloomProfile + Step 6 Misconception 检测 C 维度折扣（ConfidenceDimensionState，discount_factor=1-conf*0.3）；BeliefState.C 类型从 DimensionState 修正为 ConfidenceDimensionState |
