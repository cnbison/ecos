...
{{
  ...
  "score": 0.0到1.0之间（v0.54.0 partial credit 评分, 1.0=完全对, 0.0=完全错, 0.7=70%对），
...
- score 是 partial credit 评分, 即使 correctness=true 也可能 score<1.0 (如缺细节)；
  即使 correctness=false 也可能 score>0 (如部分对, 算法对但缺 I/O)
- score 评分锚点: 0.0=完全错, 0.3=部分对(识别位置但没说原因), 0.6=基本对(主体对但漏边界), 1.0=完全对
...
class PerceptionCritic:
    ...
    def perceive(
        ...
        # v0.54.0-c: partial credit score 字段解析
        # 老 LLM 不返回 score 时, fallback: correctness=true → score=1.0, else 0.0
        raw_score = raw.get("score")
        if raw_score is None:
            # 兼容: 老 LLM 没返回 score, 用 correctness 派生
            score = 1.0 if student_correctness else 0.0
        else:
            try:
                score = max(0.0, min(1.0, float(raw_score)))
            except (TypeError, ValueError):
                # 兼容性兜底: LLM 返回非数字
                logger.warning("PerceptionCritic: LLM 返回 score 非数字 %r, fallback 到 correctness 派生", raw_score)
                score = 1.0 if student_correctness else 0.0

        return PerceptionOutput(
            correctness=bool(raw.get("correctness", student_correctness)),
            score=score,  # v0.54.0 partial credit
