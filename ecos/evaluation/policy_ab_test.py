...
class PolicyABTest:
    ...
    def _create_fresh_bandit(policy_id: str) -> Any:
        ...
        v0.90.0-d: POMDP 工厂默认 use_learned_t_r=True + min_samples=5 (冷启动),
                   保持真 3-way A/B (linucb / thompson / pomdp+PBVI+learned T/R).
        """
        ...
        if policy_id in ("linucb", "linucb_baseline"):
            ...
            # v0.90.0-d: 默认 use_learned_t_r=True + min_samples=5 (T/R 后验 + 冷启动)
            return POMDPPolicy(
                n_arms=10,
                seed=42,
                use_pbvi=True,
                use_learned_t_r=True,
                min_samples=5,
            )
        else:
