...
[![Status](https://img.shields.io/badge/status-v0.98.1--e2e-fix-brightgreen)]()
[![Version](https://img.shields.io/badge/version-0.98.1-blue)]()
...
**创建日期**：2026-06-24
**当前版本**：v0.98.1（2026-09-06 恢复期：接线审计 → 黄金回归基建 v0.97.0 → v0.97.1 BKT 视图+L3 接线 → v0.97.2 学生自评+校准视图 → v0.97.3 A2 reconcile → v0.98.0 家长端 + Evidence/Event 注入答题流（接线审计实例 ③ 收口）+ H1/Twin 数据收集方案 → v0.98.1 fix 生产 Plugin 路径 evidence/event 落库恢复（端到端实测发现）；下一步 = 试点执行）
