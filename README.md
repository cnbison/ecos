...
**创建日期**：2026-06-24
**当前版本**：v0.98.5（2026-09-07 恢复期：接线审计 → 黄金回归基建 v0.97.0 → v0.97.1 BKT 视图+L3 接线 → v0.97.2 学生自评+校准视图 → v0.97.3 A2 reconcile → v0.98.0 家长端 + Evidence/Event 注入答题流（接线审计实例 ③ 收口）+ H1/Twin 数据收集方案 → v0.98.1 fix 生产 Plugin 路径 evidence/event 落库恢复 → v0.98.2 web-ui P0 优化（响应式 bug / 版本号 / 共享组件 / a11y / 家长建议样式）→ v0.98.3 web-ui P1 优化（教师详情页重排 + Lucide 图标 + 学生端 emoji 替换）→ v0.98.4 web-ui P1 优化②（家长端 URL 状态 + 教师班级列表移动端卡片化）→ v0.98.5 测试/生产库隔离收口 + 生产库测试数据清理（ECOS_DB_PATH 统一 + lbc 黄金数据 fixture 化，38 测试账号清空有备份）→ v0.98.6 dogfood F-03 修复（cross_subject 元探针出池，方案 A）→ v0.98.7 dogfood F-04 修复（提交按钮出结果后禁用）→ v0.98.8 dogfood F-06 修复（写代码题 rubric 补齐 12 道 + judge prompt 防幻觉）→ v0.98.9 dogfood F-07 修复（通俗化备注提交后即时刷新）→ v0.99.0 试点信号采集批次（F-05 judge 审计 / F-09 时延 / F-10 误解检测 / F-11 行为事件，四链路接通）；当前 = dogfood 二轮验证 全流程自测中，发现清单见 docs/dogfood-findings-2026-09.md）
