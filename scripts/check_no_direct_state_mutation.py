...
实现:
- v0.80: soft warning (exit 0, 仅打印警告)
- v0.81: hard block (exit 1, after TODO mutations migrated to StateEngine)
"""
import ast
import sys
...
# 行 allowlist: 这些位置在 v0.80 暂未迁移, v0.81 修复后移除
LINE_ALLOWLIST = {
    # web/api/belief.py: trajectory snapshots append + overall_confidence 直写
    ("web/api/belief.py", 175): "trajectory.snapshots.append",
    ("web/api/belief.py", 303): "overall_confidence",
    ("web/api/belief.py", 312): "overall_confidence",
    # ecos_session.py:193-198: legacy MVP 直接 theta/overall_confidence 写
    ("ecos/session/ecos_session.py", 193): "theta_mean",
    ("ecos/session/ecos_session.py", 194): "theta_mean",
    ("ecos/session/ecos_session.py", 195): "overall_confidence",
    ("ecos/session/ecos_session.py", 196): "overall_confidence",
    ("ecos/session/ecos_session.py", 197): "overall_confidence",
    ("ecos/session/ecos_session.py", 198): "overall_confidence",
    # orchestrator.py:842: state[sid].student_id = sid (allowed exception per plan)
...
def main():
    ...
    if all_violations:
        print("⚠️  发现直接 state.X = value mutation (v0.80 soft warning, v0.81 将 hard block):")
        ...
        print("allowlist: BeliefState.{__init__,to_dict,from_dict,apply_snapshot,validate,bump_version} + StateEngine.commit + BeliefUpdator.apply")
        # v0.80: soft warning (exit 0)
        # v0.81: 改 sys.exit(1) after TODO mutations migrated
        sys.exit(0)
