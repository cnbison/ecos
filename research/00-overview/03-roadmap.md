...
| **H3 双 Agent 互校抗幻觉** | 双 Agent vs 单 Agent ECE | 双 Agent ECE ≤ 0.10 | ⚠️ 双 Agent 互校未实施 | ❌ **当前数据下未通过 (V1/V2 指标反向)**; v0.69.0 重设计 confidence 指标后重跑 |
...
| **v0.59.0** | H3 验证（互校抗幻觉实证）+ 跑通降 A10 风险等级 | v0.58.0 完成 | ⚠️ 2026-07-29-30 (A 报告 60 样本 + B 报告 35 样本; 当前数据下未通过, v0.69.0 重设计指标后重跑) |
...
| 决策点 | 通过条件 | 失败动作 | 当前状态 |
|---|---|---|---|
| **M3 后：H1-H3** | CTA 预测力 + Bloom 可行 + 互校抗幻觉 | 简化为 3D + 重组 Bloom + 强化规则 | ⚠️ H2 通过, H1 待 formal AUC, **H3 当前数据下未通过 (V1/V2 指标选错), v0.69.0 重设计后重跑** |
...
| **双 Agent 互校无法抗幻觉** | 高 | H3 | — | ⚠️ 当前数据下未通过（V1/V2 指标选错），v0.69.0 重设计后重跑 |
