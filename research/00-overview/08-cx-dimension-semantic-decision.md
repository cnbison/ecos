# C / X 维度语义决策文档（PHA5-DEC-CX v1.1）

> **日期**：2026-07-23（v1.0 21:36 / v1.1 22:35 增 Bisen 调研 + 领域无关原则 + 方案 D）
> **触发**：Bisen 2026-07-23 21:28 "C和X维度的真实含义是什么？对5D的完整性是否很重要和必要？"
> **v1.1 触发**：Bisen 2026-07-23 22:25 调研 3 个权威文档 + "ECOS 应该是领域无关" 原则
> **状态**：🟡 **PENDING Bisen 决策**（v1.0 3 方案 + v1.1 方案 D 待选）
...
> - [research/deep-research/Cognitive-Digital-Twin-Deep-Research.md v2.0 §3.3 CTA 可观测证据](../deep-research/Cognitive-Digital-Twin-Deep-Research.md)
> - [research/00-overview/02-architecture.md §2.1 State Space](02-architecture.md)
...
## 11. v1.1 增: 3 个权威文档调研 + 领域无关原则 + 方案 D（Bisen 2026-07-23 22:25 触发）

### 11.1 Bisen 调研要求

Bisen 2026-07-23 22:25 要求查阅 3 个权威文件 + 强调 "ECOS 应该是领域无关" 原则：
1. `research/deep-research/Cognitive-Digital-Twin-Deep-Research.md` line 715+ "### CTA 的可观测证据"
2. `research/00-overview/02-architecture.md` line 173+ "### 2.1 State Space（状态空间）"
3. `research/30-shared-cognitive-tools/theoretical-foundations/01-cta-mathematical-foundations.md` line 54+ "### 1.2 与 ECOS CTA 的对接"

### 11.2 3 个权威文档 C/X 定义调研结果

| 文档 | C 维度定义 | X 维度定义 |
|------|-----------|-----------|
| **deep-research v2.0 §3.3 CTA 可观测证据** | **自我评估、求助行为、检查行为** | **工具调用、笔记质量、Agent 记忆** |
| **02-architecture.md §2.1 State Space** | `ConfidenceState` 认知置信度（含 misconception 折扣）| `ExternalSupportState` 外部支架 |
| **01-cta-mathematical-foundations.md §1.2 CTA 对接** | **C（Confidence / 概念联结）** — 概念联结强度（含 misconceptions）| **X（External / 元认知）** — 元认知 + 外部支持依赖 |

**3 个权威文档完全一致**：
- C = **Confidence 认知置信度**（含 self_evaluation / 求助 / 检查 / misconception 折扣）
- X = **External Support 外部支架**（含工具调用 / 笔记 / 记忆）

### 11.3 漂移根因（重新分析）

之前 v1.0 我归因于"教学法层 vs 信念引擎层"——但**真正的根因是**：

- v0.52.1 审查报告 + v0.54.0-b C 主导题设计**把 C 维度降级到"编程教学法"**（Common mistakes 调试能力）
- v0.52.1 审查报告 + v0.54.2 计划**把 X 维度降级到"编程应用"**（跨语言迁移）
- **这违反了 ECOS 5D 维度"领域无关"的设计意图**——Bisen 2026-07-23 22:25 提醒的关键点

### 11.4 领域无关原则的杀伤力

| 5D 维度 | 领域无关性 | 跨学科适用性 |
|---------|-----------|--------------|
| K 知识掌握 | ✅ 通用 | 数学/语文/英语/物理/化学都通用 |
| P 程序技能 | ✅ 通用 | 写代码/写作/解题/实验都通用 |
| S 策略能力 | ✅ 通用 | 选算法/选思路/选策略都通用 |
| **C 认知置信度** | ✅ **通用** | **自我评估/求助/检查跨学科都通用** |
| **X 外部支架** | ✅ **通用** | **工具调用/笔记/记忆跨学科都通用** |
| ❌ Common mistakes 调试 | ❌ **编程特定** | 数学/语文/英语/物理/化学**不适用** |
| ❌ 跨语言迁移 | ❌ **编程特定** | 编程领域内部用，不跨学科 |

**关键洞察**：
- 3 个权威文档定义的 C/X 是**领域无关**的——任何学科都能用
- v0.54.1 把 C 降级到"Common mistakes 调试"——**编程领域特定**，不跨学科
- v0.54.2 把 X 降级到"跨语言迁移"——**编程领域特定**，不跨学科
- **Common mistakes / 跨域迁移是"应用层扩展"**（不是 5D 核心）

### 11.5 新方案 D: 领域无关 + 恢复权威定义

#### 核心决策

1. **C 维度 = "Confidence 认知置信度"**（含 misconception 折扣）
   - **领域无关**：自我评估 / 求助行为 / 检查行为 / misconception 检测 跨学科通用
   - 典型题型：自我评估题、求助决策题、检查行为题、misconception 检测题

2. **X 维度 = "External Support 外部支架"**（含工具调用 / 笔记 / 记忆）
   - **领域无关**：工具 / 笔记 / Agent 记忆跨学科通用
   - 典型题型：工具选择题、笔记质量评估题、记忆使用决策题

3. **Common mistakes 调试 / 跨域迁移 = 应用层扩展**
   - **不是 5D 核心维度**
   - 编程领域作为 `programming_debug_score` / `cross_language_score` 字段（独立于 5D）
   - 跨学科时这些字段不应用

#### 优点

1. ✅ **跟 3 个权威文档完全一致**（deep-research v2.0 + 02-arch + 01-cta）
2. ✅ **ECOS 5D 真正领域无关**——跨数学/语文/英语/物理/化学都通用
3. ✅ **学术严谨性大幅提升**——恢复原始设计意图
4. ✅ **v0.54.1-c 端到端测试的 lbc001 C θ=-0.07 重新解释**——这是"lbc001 自我评估不准"，不是"调试能力弱"
5. ✅ **Common mistakes / 跨域迁移保留为应用层扩展**——不污染 5D 核心，但保留 ECOS 编程领域能力

#### 风险

- 🟡 v0.54.1 已落地的 20 道 PB-C 主导题**不能直接用于 5D C 维度核心**——需要处理（见 §11.6）
- v0.54.2 计划需要重新设计
- lbc001 之前 C 主导题答题数据需要重新评估（之前 -0.07 是"调试能力"评估，不是"Confidence"评估）

### 11.6 v0.54.1 已落地 20 道 PB-C 主导题处理

**3 个处理选项对比**：

| 选项 | 操作 | 优缺点 | 工作量 |
|------|------|--------|--------|
| **D-1 完全废弃** | 删除 20 道 PB-C，重新设计 20 道 C-confidence（领域无关）| 完全符合 3 权威文档 + 领域无关，但 lbc001 5 题数据废弃 | 🔴 5-7 天 |
| **D-2 保留为应用层扩展** | 20 道 PB-C 改名"编程领域应用层"，**新增** 5+ 道 C-confidence 样题（领域无关）| 保留 v0.54.1 工作量 + lbc001 数据价值，5D 核心 4/5 + 编程扩展 1/1 | 🟢 1-2 天 |
| **D-3 改造为 C-confidence** | 把"调试题"重设计为"自我评估题"（基于 lbc001 行为推断）| 复用 Q 矩阵结构，但题型设计复杂 | 🟡 3-5 天 |

**推荐 D-2**：
1. v0.54.1 已落地的 20 道 PB-C 保留——作为**编程领域应用层扩展**，评估 `programming_debug_score`（独立字段，不属于 5D 核心）
2. **新增** 5+ 道 C-confidence 主导题（领域无关）——基于 self_evaluation / 求助 / 检查 / misconception 检测
3. ECOS 5D 完整性 = **5/5**（K/P/S/C-confidence/X-external-support 全部真评估）
4. ECOS 编程领域扩展 = **1/1**（programming_debug_score 评估）

**v0.54.1-c 端到端测试数据重新解释**：
- lbc001 C θ=0.2145 → -0.0685（5 道 PB-C partial credit 0.68 平均）
- **不是** "调试能力中等偏下"——是 **"programming_debug_score 中等偏下"**（独立字段）
- 5D 核心 C 维度（Confidence）= 0.2145（仍是占位值，待 C-confidence 主导题真评估）

### 11.7 C-confidence 主导题 5 道样题设计

| ID | Bloom | 题型 | 题目示例 | a_specialized (K/P/S/C/X) |
|----|-------|------|----------|---------------------------|
| PC-C01 | L3 | 自我评估 | "你答对这道题的可能性？70%/50%/30%" — LLM 评判 self_evaluation | [0.2, 0.2, 0.3, **1.1**, 0.2] |
| PC-C02 | L4 | 求助决策 | "遇到难题你打算？1. 看笔记 2. 问老师 3. 独立思考 4. 直接答" | [0.2, 0.2, 0.3, **1.2**, 0.2] |
| PC-C03 | L5 | 检查行为 | "答完代码你是否会检查？1. 不检查 2. 简单检查 3. 完整测试" | [0.2, 0.2, 0.3, **1.0**, **0.3**] |
| PC-C04 | L5 | misconception 检测 | "range(1, 5) 包括 5 吗？1. 包括 2. 不包括" | [0.3, 0.2, 0.2, **1.3**, 0.0] |
| PC-C05 | L6 | 综合 Confidence | 多行为综合评估（自我评估 + 求助 + 检查 + misconception）| [0.2, 0.2, 0.3, **1.2**, 0.2] |

**领域无关验证**：
- PC-C01（自我评估）—— 数学/语文/英语/物理/化学都适用
- PC-C02（求助决策）—— 跨学科通用
- PC-C03（检查行为）—— 跨学科通用
- PC-C04（misconception 检测）—— 跨学科通用（misconception 是心理学通用概念）
- PC-C05（综合 Confidence）—— 跨学科通用

### 11.8 X-external-support 主导题 5 道样题设计（v0.54.2 计划）

| ID | Bloom | 题型 | 题目示例 | a_specialized (K/P/S/C/X) |
|----|-------|------|----------|---------------------------|
| PC-X01 | L3 | 工具选择 | "调试代码用什么工具？1. print 2. debugger 3. 单元测试 4. 直接看" | [0.2, 0.2, 0.2, 0.2, **1.0**] |
| PC-X02 | L4 | 笔记质量 | "笔记应包含？1. 代码 2. 思路 3. 易错点 4. 全部" | [0.2, 0.2, **0.4**, 0.2, **1.0**] |
| PC-X03 | L5 | 记忆使用 | "是否查之前类似题？1. 是 2. 否" | [0.2, 0.2, 0.2, 0.2, **1.1**] |
| PC-X04 | L5 | 支架依赖 | "用 worked example 还是独立做？" | [0.2, 0.2, 0.2, 0.3, **1.0**] |
| PC-X05 | L6 | 综合 External Support | 多维度支架依赖评估 | [0.2, 0.2, 0.3, 0.3, **1.1**] |

**领域无关验证**：
- PC-X01（工具选择）—— 数学（计算器/几何工具）/ 语文（字典/语法书）/ 英语（翻译工具）/ 物理（实验仪器）/ 化学（实验工具）都适用
- PC-X02（笔记质量）—— 跨学科通用
- PC-X03（记忆使用）—— 跨学科通用
- PC-X04（支架依赖）—— 跨学科通用
- PC-X05（综合 External Support）—— 跨学科通用

### 11.9 实施步骤

| 版本 | 内容 | 工作量 |
|------|------|--------|
| **v0.54.1-e**（当前 commit）| 决策文档更新（v1.0 → v1.1）加 §11 + 方案 D | 🟢 已完成 |
| **v0.54.1-f**（本周）| 5 道 C-confidence 主导题样题设计 + 端到端测试 + 保留 20 道 PB-C 为编程扩展 | 🟡 2-3 天 |
| **v0.54.1-g**（下周）| 扩 20+ 道 C-confidence 主导题 + Q 矩阵 JSON 追加 | 🟡 1 周 |
| **v0.54.2**（后续）| 5+ 道 X-external-support 主导题样题 + 端到端测试 | 🟡 2-3 天 |
| **v0.54.2+** | 扩 20+ 道 X-external-support 主导题 + 字段解耦（programming_debug_score）| 🟡 1 周 |
| **v0.55.0** | pytest 套件 + 跨学科迁移验证（数学/语文/英语） | 🟡 1 周 |

### 11.10 5D 完整性最终评估

| 维度 | 含义 | 领域无关 | 改造前 | 改造后 |
|------|------|----------|--------|--------|
| K | Knowledge 知识掌握 | ✅ | ✅ 真评估 | ✅ 真评估 |
| P | Procedure 程序技能 | ✅ | ✅ 真评估 | ✅ 真评估 |
| S | Strategy 策略能力 | ✅ | ✅ 真评估 | ✅ 真评估 |
| **C** | **Confidence 认知置信度** | ✅ | ❌ 占位 | ✅ **真评估 (C-confidence 主导题)** |
| **X** | **External Support 外部支架** | ✅ | ❌ 占位 | ✅ **真评估 (X-external-support 主导题)** |
| **5D 完整性** | | | **3/5** | **5/5** ✅ |
| **(扩展) programming_debug_score** | 编程调试能力 | ❌ 编程特定 | n/a | ✅ 评估 (20 道 PB-C 保留) |

**5D 评估完整性 = 5/5（领域无关）** + **编程领域扩展 = 1/1（应用层）** = **ECOS 真正"领域无关 + 应用层扩展"双层架构**

### 11.11 决策对比

| 维度 | 方案 A 激进 | 方案 B 保守 | 方案 C 中间路线 | **方案 D 领域无关**（v1.1 推荐）|
|------|------------|------------|----------------|----------------------------|
| C 维度 | 改名 Common mistakes | 维持 Confidence + 文档修复 | 双含义 + 字段解耦 | **恢复 Confidence 权威定义** |
| X 维度 | 改名 跨域迁移 | 维持 External Support | 双含义 + 字段解耦 | **恢复 External Support 权威定义** |
| 5D 领域无关 | ❌（编程领域特定）| ❌（含义模糊）| ⚠️（双含义）| ✅ **完全领域无关** |
| v0.54.1 20 道 PB-C | 🔴 废弃 | ⚠️ 含义模糊 | 🟡 字段解耦 | ✅ **保留为编程扩展** |
| v0.54.2 20 道 X 主导题 | 🔴 废弃 | n/a | 🟡 字段解耦 | ✅ **重新设计 X-external-support** |
| 工作量 | 🔴 1-2 周 | 🟢 文档 | 🟡 2-3 天 | 🟢-🟡 **1-2 周** |
| 风险 | 🔴 高 | 🟢 低 | 🟡 中 | 🟡 中 |
| 学术严谨性 | ⚠️ 跟 v2.0 冲突 | ⚠️ 文档漂移 | 🟡 双含义复杂 | ✅ **跟 v2.0 一致** |

### 11.12 Bisen 最终决策（v1.1）

- [ ] **方案选择**：A / B / C / **D**？
- [ ] **v0.54.1 20 道 PB-C 处理**：D-1 完全废弃 / D-2 保留为编程扩展 / D-3 改造为 C-confidence？
- [ ] **实施优先级**：v0.54.1-f (C-confidence 5 道样题) → v0.54.1-g (扩 20+ 道) → v0.54.2 (X-external-support)？

---

## 12. v1.1 决策记录（Bisen 待选）

### 12.1 Bisen 2026-07-23 22:25 反馈

- ✅ 调研 3 个权威文档：C/X 定义是 Confidence / External Support
- ✅ 强调 "ECOS 应该是领域无关" 原则
- ✅ Common mistakes 调试 / 跨域迁移是**编程领域应用层扩展**（不是 5D 核心）
- ✅ 推荐 **方案 D**（恢复权威定义 + 领域无关原则）

### 12.2 Mavis 2026-07-23 22:35 反思

- 关键发现：**v0.54.1 文档漂移根因是"领域降级"**——把领域无关的 5D 维度降级到编程领域特定
- 修正之前 v1.0 归因"教学法层 vs 信念引擎层"——**真正的冲突是"领域无关 vs 编程领域特定"**
- 方案 D 让 ECOS 5D **真正领域无关** + 编程应用层扩展——双层架构更清晰
- v0.54.1 20 道 PB-C 不废弃（D-2）——保留为编程领域应用层扩展
- v0.54.1-c 端到端测试数据重新解释：lbc001 C θ=-0.07 是 "programming_debug_score"，不是 5D C 维度
- 5D 评估完整性 5/5 + 编程扩展 1/1 = **ECOS 真正"领域无关 + 应用层扩展"双层架构**

### 12.3 决策影响

- v0.54.1 已落地的 20 道 PB-C **保留为编程领域应用层扩展**（不废弃）
- v0.54.1 后续：新增 C-confidence 主导题（领域无关）
- v0.54.2：重新设计 X-external-support 主导题（领域无关）
- v0.55.0：跨学科迁移验证（数学/语文/英语）
- 整体 ECOS 5D 架构：5D 核心领域无关 + 编程应用层扩展

### 12.4 文档维护规则

- 5D 维度定义以 3 个权威文档为准（deep-research v2.0 + 02-arch + 01-cta）
- 应用层扩展（编程 / 数学 / 语文 等领域特定）独立于 5D 核心
- CI gate 6: 5D 维度定义跨文档一致性 + 领域无关性检查

---

**创建日期**：2026-07-23
**v1.0**：21:36（3 方案 A/B/C 决策文档）
**v1.1**：22:35（Bisen 调研 + 领域无关原则 + 方案 D + 20 道 PB-C 处理）
**维护者**：Bisen & Mavis
**下次更新**：Bisen 决策后（v0.54.1-f）
